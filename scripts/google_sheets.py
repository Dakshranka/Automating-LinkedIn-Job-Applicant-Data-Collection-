...
def check_duplicate_in_drive(pdf_filename):
    ...
    try:
        ...
        if files:
            print(f"⚠️ Resume '{pdf_filename}' already exists in Google Drive.")
...
def upload_to_sheets(name, email, phone, resume_link, existing_data):
    ...
    if (name, email) in existing_data:
        print(f"⚠️ Duplicate entry for {name} ({email}) found. Skipping Google Sheets upload.")
        return
...
def process_resumes():
    ...
    for pdf_file in pdf_files:
        ...
        if not text.strip():
            print(f"⚠️ Skipping {pdf_file} due to empty text extraction.")
            continue
